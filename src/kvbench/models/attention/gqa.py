...
class GQA(nn.Module):
    ...
    def forward_with_cache_timed(
        ...
        kv_len = k.size(2)
        k_expand = k.repeat_interleave(self.group_size, dim=1)
        v_expand = v.repeat_interleave(self.group_size, dim=1)

        scores = torch.einsum("bhtd,bhsd->bhts", q, k_expand) * self.scale
        mask = self._causal_mask(t, kv_len, cache_len, x.device)
        scores = scores.masked_fill(~mask[None, None, :, :], torch.finfo(scores.dtype).min)

        attention_start = perf_counter()
        attn = F.softmax(scores, dim=-1)
        y = torch.einsum("bhts,bhsd->bhtd", self.dropout(attn), v_expand)
